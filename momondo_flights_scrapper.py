...
def momondo_flights_search_return(departure_city, destination_city, departure_date, days_of_stay, agent):
    return_date = datetime.strptime(departure_date, '%Y-%m-%d').date() + timedelta(days_of_stay)
    return_date = return_date.strftime('%Y-%m-%d')

    url = "https://www.momondo.pl/flight-search/" + departure_city + "-" + destination_city + "/" + departure_date \
    ...
    driver.get(url)
    momondo_page = soup(driver.page_source, "html.parser")

    start_flight = momondo_page.body.find_all('li', attrs={'class': 'flight with-gutter'})
    ...
    end_flight = momondo_page.body.find_all(lambda tag: tag.name == 'li' and
    ...
    flight_price = []
    div_price = momondo_page.find_all('div', attrs={'class': 'Flights-Results-FlightPriceSection right-alignment sleek'})
    ...
    df_momondo = pd.DataFrame({'start_departure_time': start_departure_time, 'start_arrival_time': start_arrival_time,
...
def momondo_flights_search_oneway(departure_city, destination_city, departure_date, agent):
    ...
    start_flight = momondo_page.body.find_all('li', attrs={'class': 'flight'})
